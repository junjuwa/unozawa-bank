...
続けて以下を順番に適用してください（0001 → 0002 → 0003 → 0004 → 0005 の順を守ること）：
...
- `0005_goals.sql`（design.mdに無かった「目標（ほしいもの）」を保持する `goals` テーブルを追加。お金を動かさないため子が自分の行を直接書込めるRLS）

### おしごと・目標の実運用に必要な準備

`/jobs` `/approvals` は子・親ともログイン済みなら実DBの `job_tasks` / `job_requests` を使う。**おしごとの単価マスタ（`job_tasks`）の追加・編集・削除は設定画面（`/settings`）から行える**ため、SQL Editorでの手動INSERTは不要（0006以前の名残りで以下のSQLも動作はする）：
...
`/goals`（目標）・`/grow`（ふやす）・`/settings`の基本給/投資金利/満期日数/やくそく は実ログイン済みならすべて実DBに接続済み。「今すぐ支給（テスト）」ボタンは `pay_base_salary()` がservice role専用のため実モードでは表示されない（cron任せ、またはSQL Editorから手動で `select public.pay_base_salary();` をservice roleとして実行）。

`/dashboard`（親の横断ダッシュボード）・`/history`（子の「つかった きろく」）・親が記録する支出（`spend_money` RPC）も実ログイン済みならすべて実DBに接続済み（フェーズ7）。ダッシュボードの「今週ためた額」はAsia/Tokyoの日曜0時からの`transactions`（`to_kind='save'`）集計。
